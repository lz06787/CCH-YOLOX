...
class BaseDeformConv(nn.Module):
    ...
    def __init__(
        ...
        #     bias=bias,
        # )
        self.conv = DeformConv2d(in_channels, out_channels, kernel_size=ksize, stride=stride, padding=pad, groups=groups, bias=bias)
        #self.bn = nn.BatchNorm2d(out_channels)
        self.bn = nn.GroupNorm(num_groups=16, num_channels=out_channels)
        self.act = get_activation(act, inplace=True)

    def forward(self, x, **kwargs):
        return self.act(self.bn(self.conv(x, **kwargs)))

    def fuseforward(self, x):
        return self.act(self.conv(x))

class YOLOXHead_DCN2(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        DeformConv = BaseDeformConv
        self.offset = nn.Conv2d(int(256 * width), 27, kernel_size=3, stride=1, padding=1)
        ...
        for i in range(len(in_channels)):
            ...
            # self.cls_convs.append(DeformConv(
            ...
            #                 act=act,
            #             ))
            # self.reg_convs.append(DeformConv(
            #                 in_channels=int(256 * width),
            #                 out_channels=int(256 * width),
            #                 ksize=3,
            #                 stride=1,
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            offset_mask = self.offset(cls_x)
            offset = offset_mask[:, :18, :, :]
            mask = offset_mask[:, 18:, :, :].sigmoid()
            conv_args0 = dict(offset=offset, mask=mask)
            cls_feat0 = cls_conv[0](cls_x, **conv_args0)

            offset_mask = self.offset(cls_feat0)
            offset = offset_mask[:, :18, :, :]
            mask = offset_mask[:, 18:, :, :].sigmoid()
            conv_args1 = dict(offset=offset, mask=mask)
            cls_feat = cls_conv[1](cls_feat0, **conv_args1)

            cls_output = self.cls_preds[k](cls_feat)

            offset_mask = self.offset(reg_x)
            offset = offset_mask[:, :18, :, :]
            mask = offset_mask[:, 18:, :, :].sigmoid()
            conv_args2 = dict(offset=offset, mask=mask)
            reg_feat0 = reg_conv[0](reg_x, **conv_args2)

            offset_mask = self.offset(reg_feat0)
            offset = offset_mask[:, :18, :, :]
            mask = offset_mask[:, 18:, :, :].sigmoid()
            conv_args3 = dict(offset=offset, mask=mask)
            reg_feat = reg_conv[1](reg_feat0, **conv_args3)
