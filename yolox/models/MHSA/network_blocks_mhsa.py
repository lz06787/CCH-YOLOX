...
import warnings
from .MHSA import MHSA_V1, MHSA_V2, MHSA_V3
...
class MHSAConv(nn.Module):
    ...
    def __init__(
        ...
        self.bn = nn.BatchNorm2d(out_channels)
        self.act = get_activation(act, inplace=True)
        self.mhsa = MHSA(in_channels)

    def forward(self, x):
        return self.act(self.bn(self.mhsa(x)))

    def fuseforward(self, x):
        return self.act(self.mhsa(x))
...
class Bottleneck_MHSA(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = MHSAConv(hidden_channels, hidden_channels)
        self.conv3 = BaseConv(hidden_channels, in_channels, 1, stride=1, act=act)
