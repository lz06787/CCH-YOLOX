...
from yolox.models.FaPN import FeatureAlignModule

from .darknet import CSPDarknet
from .network_blocks import BaseConv, CSPLayer, DWConv
from .FaPN import FeatureSelectionModule

class YOLONOFPN12(nn.Module):
    """
    ...
    def __init__(
        ...
        self.fsm1 = FeatureSelectionModule(int(in_channels[1]* width), int(in_channels[1]* width))
        self.fsm2 = FeatureSelectionModule(int(in_channels[0]* width), int(in_channels[0]* width))
        self.fsm3 = FeatureSelectionModule(int(in_channels[2]* width), int(in_channels[2]* width))
    ...
    def forward(self, input):
        ...
        x1_sel = self.fsm1(x1)
        f_out0 = torch.cat([f_out0, x1_sel], 1)  # 512->1024/16
        ...
        x2_sel = self.fsm2(x2)
        f_out1 = torch.cat([f_out1, x2_sel], 1)  # 256->512/8
        pan_out2 = self.C3_p3(f_out1)  # 512->256/8

        #f_out3 = self.maxpool(x2)
        f_out3 = self.buconv1(x1)
        x0_sel = self.fsm3(x0)
        f_out3 = torch.cat([f_out3, x0_sel], 1)
