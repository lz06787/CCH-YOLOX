...
class YOLOXHead_likefocs(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            ...
            for head, output in enumerate(outputs):
                ...
                loss2 += loss
                iou_loss2 += iou_loss
                conf_loss2 += conf_loss
                cls_loss2 += cls_loss
                l1_loss2 += l1_loss
                num_fg2 += num_fg
            
            return (loss2/3, iou_loss2/3, conf_loss2/3, cls_loss2/3, l1_loss2/3, num_fg2)
    ...
    def get_losses(
        ...
        x_shifts = x_shifts_total[head]
        y_shifts = y_shifts_total[head]
        #expanded_strides = torch.cat(expanded_strides, 1)
        expanded_strides = expanded_strides_total[head]
        if self.use_l1:
            #origin_preds = torch.cat(origin_preds, 1)
            origin_preds = origin_preds_total[head]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_bboxes_per_image_total = labels[batch_idx, :num_gt, 1:5]
                gt_classes_total = labels[batch_idx, :num_gt, 0]
                
                # 对gt的范围进行划分
                if head == 0:
                    head_mask = (gt_bboxes_per_image_total[:,2]*gt_bboxes_per_image_total[:,3] < 32*32).squeeze()
                    gt_bboxes_per_image = gt_bboxes_per_image_total[head_mask]
                    gt_classes = gt_classes_total[head_mask]

                elif head == 1:
                    head_mask = ((gt_bboxes_per_image_total[:,2]*gt_bboxes_per_image_total[:,3] > 32*32) * (gt_bboxes_per_image_total[:,2]*gt_bboxes_per_image_total[:,3] < 96*96)).squeeze()
                    gt_bboxes_per_image = gt_bboxes_per_image_total[head_mask]
                    gt_classes = gt_classes_total[head_mask]
                else:
                    head_mask = (gt_bboxes_per_image_total[:,2]*gt_bboxes_per_image_total[:,3] > 96*96).squeeze()
                    gt_bboxes_per_image = gt_bboxes_per_image_total[head_mask]
                    gt_classes = gt_classes_total[head_mask] 
                ...
                # 有一些gt的shape为(1,1,4)
                elif len(gt_bboxes_per_image.shape) == 3:
                    cls_target = outputs.new_zeros((0, self.num_classes))
                    reg_target = outputs.new_zeros((0, 4))
                    l1_target = outputs.new_zeros((0, 4))
                    obj_target = outputs.new_zeros((total_num_anchors, 1))
        ...
        #================================================#
        loss_iou = (
            self.iou_loss(bbox_preds.contiguous().view(-1, 4)[fg_masks], reg_targets)
        ).sum() / num_fg
        #================================================#
        ...
        #================================================#
        loss_obj = (
            self.bcewithlog_loss(obj_preds.contiguous().view(-1, 1), obj_targets)
        ).sum() / num_fg
        ...
        loss_cls = (
            self.bcewithlog_loss(
                cls_preds.contiguous().view(-1, self.num_classes)[fg_masks], cls_targets
            )
        ).sum() / num_fg
        #================================================#
        ...
        if self.use_l1:
            loss_l1 = (
                self.l1_loss(origin_preds.contiguous().view(-1, 4)[fg_masks], l1_targets)
            ).sum() / num_fg
