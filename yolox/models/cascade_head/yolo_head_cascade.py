...
class YOLOXHead_CASCADE(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                output = self.get_output_and_grid2(
                    output, output2, k, stride_this_level, xin[0].type()
                )
    ...
    def get_output_and_grid2(self, output, output2, k, stride, dtype):
        grid = output[...,:2]
        batch_size = output.shape[0]
        n_ch = 5 + self.num_classes
        hsize, wsize = output2.shape[-2:]
        ...
        output2[..., :2] = (output2[..., :2] + grid) * stride
        output2[..., 2:4] = torch.exp(output2[..., 2:4]) * stride
        return output2
