...
from yolox.models.dcn.darknet_dcn import CSPDarknet_DCN
from .network_blocks import BaseConv, CSPLayer, DWConv
from yolox.models.dcn.network_blocks_dcn import BaseDeformConv, CSPLayer_DCN
class YOLOPAFPN_DCN(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.backbone = CSPDarknet_DCN(depth, width, depthwise=depthwise, act=act)
        self.in_features = in_features
        self.in_channels = in_channels
        Conv = DWConv if depthwise else BaseDeformConv

        self.upsample = nn.Upsample(scale_factor=2, mode="nearest")
        self.lateral_conv0 = BaseConv(
            int(in_channels[2] * width), int(in_channels[1] * width), 1, 1, act=act
        )
        self.C3_p4 = CSPLayer_DCN(
