...
class Bottleneck_dilateA(nn.Module):
    # Standard bottleneck
    def __init__(
        self,
        in_channels,
...
class Bottleneck_dilateB(nn.Module):
    # Standard bottleneck
    def __init__(
        self,
        in_channels,
        ...
        self.conv1 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        self.conv2 = Conv(hidden_channels, out_channels, 3, stride=1, act=act)
        self.conv3 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        self.use_add = shortcut and in_channels == out_channels
...
class CSPLayer_dilate(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n):
            if _== 0:
                ...
                module_list.append(Bottleneck_dilateA(
                        hidden_channels, hidden_channels, shortcut, 1.0, depthwise, act=act
            ))
