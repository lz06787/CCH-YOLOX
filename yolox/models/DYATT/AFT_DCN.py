...
class AFT_FULL(nn.Module):
    ...
    def __init__(self, d_model,simple=False):
        ...
        # n = 256
        self.fc_q = nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=1, stride=1,padding=0, bias=False)
        self.fc_k = nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=1, stride=1,padding=0, bias=False)
        self.fc_v = nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=1, stride=1,padding=0, bias=False)
        # if(simple):
    ...
    def forward(self, input):
        ...
        self.position_biases=nn.Parameter(torch.ones((c,c))).cuda()

        q = self.fc_q(input) #bs,c,h*w
        k = self.fc_k(input).view(1,bs,c,h*w) #1,bs,c,h*w
        v = self.fc_v(input).view(1,bs,c,h*w) #1,bs,c,h*w
        
        numerator=torch.sum(torch.exp(k+self.position_biases.view(c,1,-1,1))*v,dim=2) #c,bs,h*w
        denominator=torch.sum(torch.exp(k+self.position_biases.view(c,1,-1,1)),dim=2) #c,bs,h*w

        out=(numerator/denominator) #c,bs,h*w
        out=q*(out.permute(1,0,2).view(bs,c,h,w)) #bs,c,h,w
        return out
