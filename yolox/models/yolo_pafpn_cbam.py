...
class YOLOPAFPN_CBAM(nn.Module):
    """
    ...
    def __init__(
        ...
        self.cbam0 = CBAMBlock(int(in_channels[1] * width))
        self.cbam1 = CBAMBlock(int(in_channels[0] * width))
        self.cbam2 = CBAMBlock(int(in_channels[1] * width))
    def forward(self, input):
        ...
        pan_out2_2 = self.cbam1(pan_out2) #128/52->128/52

        p_out1 = self.bu_conv2(pan_out2_2)  # 128/52->128/26
        p_out1 = torch.cat([p_out1, fpn_out1], 1)  #128/26->256/26
        pan_out1 = self.C3_n3(p_out1)  # 256/26->256/26

        pan_out1_1 = self.cbam2(pan_out1) #256/26->256/26

        p_out0 = self.bu_conv1(pan_out1_1)  #256/26->256/13
