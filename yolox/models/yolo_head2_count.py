...
class YOLOXHead2COUNT(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        self.bcewithlog_loss2 = nn.BCEWithLogitsLoss(reduction='mean')
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                #         a = output[i][...,2] < 32
                #         b = output[i][...,3] < 32
                #         zero = torch.zeros_like(output[i])
                #         c = (a*b).unsqueeze(1)
                #         c = torch.repeat_interleave(c, repeats=9, dim=1)
                #         d = torch.where(c==True, output[i], zero) 
                #         if i == 0:
                #             small = d
                #         elif i == 1:
                #             small = torch.stack([small, d], dim = 0)
                #         else:
                #             d = d.unsqueeze(0)
                #             small = torch.cat((small , d), dim=0)
                #     output = small
                ...
                for i in range(output.shape[0]):
                    a = output[i][...,2] * output[i][...,3]
                    large_num = torch.tensor([torch.sum(a > 96*96)])
                    median_num = torch.tensor([torch.sum((a >= 32*32) * (a <= 96*96))])
                    small_num = torch.tensor([torch.sum(a < 32*32)])
                    if i == 0:
                        large = large_num
                        median = median_num
                        small = small_num
                    else:
                        large = torch.cat((large_num, large), 0)
                        median = torch.cat((median_num, median), 0)
                        small = torch.cat((small_num, small), 0)
                print('batch: %s'%k)
                print(large)
                print(median)
                print(small)
                #zero = torch.zeros_like(large)
                #self.loss_lz = self.bcewithlog_loss2(large.float(), zero.float())
