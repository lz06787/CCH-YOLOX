...
from yolox.models.cbam import ChannelAttention,SpatialAttention
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        #self.ca1 = ChannelAttention(hidden_channels)
        #self.sa1 = SpatialAttention()

        # self.ca2 = ChannelAttention(hidden_channels)
    ...
    def forward(self, x):
        ...
        if self.use_add:
            shortcut = x0 + x
            #shortcut = shortcut*self.sa1(shortcut)
            #shortcut = shortcut*self.sa1(shortcut)
            y = y + shortcut
...
class CSPLayer(nn.Module):
    ...
    def __init__(
        ...
        module_list = [
            Bottleneck(
                hidden_channels, hidden_channels, shortcut, 1, depthwise, act=act
            )
            for _ in range(n)
        ]
