...
class ChannelAttention_Group(nn.Module):
    def __init__(self, in_planes, ratio=16, channels_per_group=1):
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        hidden_channels = int(out_channels * 1)
        Conv = DWConv if depthwise else BaseConv
        self.conv1 = BaseConv(in_channels, hidden_channels, 1, stride=1, act=act)
        self.conv2 = Conv(hidden_channels, out_channels, 3, stride=1, act=act)

        self.use_add = shortcut and in_channels == out_channels
        self.left_part = in_channels
    def forward(self, x):
        x0 = self.conv1(x)
        y = self.conv2(x0)
        if self.use_add:
...
class CSPLayer(nn.Module):
    ...
    def __init__(
        ...
        module_list = [
            Bottleneck(
                hidden_channels, hidden_channels, shortcut, 0.5, depthwise, act=act
            )
            for _ in range(n)
        ]
        self.m = nn.Sequential(*module_list)

        self.channelattention = ChannelAttention_Group(in_channels, channels_per_group=16)
        self.channels_per_group = 16
        self.groups = in_channels//self.channels_per_group
