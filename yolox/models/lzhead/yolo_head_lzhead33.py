...
class YOLOXHead_LZHEAD3(nn.Module):
    def __init__(
        ...
        self.cls_se = nn.ModuleList()
        self.reg_se = nn.ModuleList()
        ...
        for i in range(len(in_channels)):
            ...
            self.convlist1.append(BaseConv(
                int(256 * width),
                int(256 * width),
                ksize=1,
                stride=1
            ...
            self.convlist2.append(BaseConv(
                int(256 * width),
                int(256 * width),
                ksize=1,
                stride=1))

            self.convlist3.append(BaseConv(
                int(256 * width),
                int(256 * width),
                ksize=1,
                stride=1))
            ...
            # ))
            self.cls_se.append(
                EffectiveSELayer(int(256 * width), 3, 3*8)
            )


            # self.reg_downsample.append(BaseConv(
            #     int(256 * width) * 3,
            #     int(256 * width),
            #     ksize=1,
            #     stride=1
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            x = self.convlist1[k](x)
            x1 = x
            x = self.convlist2[k](x)
            x1 = torch.cat([x1, x], dim=1)
            x = self.convlist3[k](x)
            x1 = torch.cat([x1, x], dim=1)

            cls_x = x1
            reg_x = x1

            cls_x = self.cls_se[k](cls_x)
            cls_feat = cls_conv(cls_x)
            # 通道数为4 (bs,4,w,h)
            cls_output = self.cls_preds[k](cls_feat)

            reg_x = self.reg_se[k](reg_x)
            reg_feat = reg_conv(reg_x)
            # 通道数为4 (bs,4,w,h)
            reg_output = self.reg_preds[k](reg_feat)

            reg_offset = self.reg_offset_conv[k](reg_output)
            reg_output = self.deform[k](reg_output.contiguous(), reg_offset)
