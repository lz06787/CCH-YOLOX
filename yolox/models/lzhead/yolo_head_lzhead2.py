...
from .lzhead2 import LZHead2
from .cbam import EffectiveSELayer

class YOLOXHead_LZHEAD2(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        self.lzhead = LZHead2(in_channels=int(256 * width))
        ...
        # self.reg_downsample = nn.ModuleList()
        ...
        for i in range(len(in_channels)):
            ...
            # ))
            self.cls_se.append(
                EffectiveSELayer(int(256 * width))
            )
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        x["x1"] = x1
        x["x0"] = x0
        lzhead_out = self.lzhead(x)
        xin = [lzhead_out[0], lzhead_out[1], lzhead_out[2]]
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            # cls_x = self.cls_se[k](cls_x)*cls_x+cls_x
            # cls_x = self.cls_downsample[k](cls_x)
            cls_feat = cls_conv(cls_x)
            cls_feat = self.cls_se[k](cls_feat)*cls_feat+cls_feat
            cls_output = self.cls_preds[k](cls_feat)

            # reg_x = self.reg_se[k](reg_x)*reg_x
            # reg_x = self.reg_downsample[k](reg_x)
            reg_feat = reg_conv(reg_x)
            reg_feat = self.reg_se[k](reg_feat)*reg_feat
